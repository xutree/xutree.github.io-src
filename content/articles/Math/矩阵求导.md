...
${\boldsymbol y}=
    {\begin{bmatrix}
        ...
        \\y_{m}\\
    \end{bmatrix}
}\Longrightarrow\frac{\partial {\boldsymbol y}}{\partial x} =
\begin{bmatrix}
...
矩阵标量 $f({\boldsymbol X})$ 在矩阵 ${\boldsymbol Y}$ 方向上的方向导数为：$\nabla_{\boldsymbol Y} f = \operatorname{tr} \left(\frac{\partial f}{\partial {\boldsymbol X}} {\boldsymbol Y}\right)$
...
**例题3**：线性回归问题。$l = \|{\boldsymbol X}\boldsymbol{w}- \boldsymbol{y}\|^2$。求 $\boldsymbol{w}$ 的最小二乘估计，即求 $\frac{\partial l}{\partial \boldsymbol{w}}$ 的零点。其中 $\boldsymbol{y}$ 是 $m\times 1$ 列向量，${\boldsymbol X}$ 是 $m\times n$ 矩阵，$\boldsymbol{w}$ 是 $n\times 1$ 列向量，$l$ 是标量。

**解答**：

改写模平方表达式：$l = ({\boldsymbol X}\boldsymbol{w}- \boldsymbol{y})^{\text{T}}({\boldsymbol X}\boldsymbol{w}- \boldsymbol{y})$

全微分：$dl = ({\boldsymbol X}d\boldsymbol{w})^\text{T}({\boldsymbol X}\boldsymbol{w}-\boldsymbol{y})+({\boldsymbol X}\boldsymbol{w}-\boldsymbol{y})^\text{T}({\boldsymbol X}d\boldsymbol{w}) = 2({\boldsymbol X}\boldsymbol{w}-\boldsymbol{y})^\text{T}{\boldsymbol X}d\boldsymbol{w}$

对照全微分公式得到：$\frac{\partial l}{\partial \boldsymbol{w}}= (2({\boldsymbol X}\boldsymbol{w}-\boldsymbol{y})^\text{T}{\boldsymbol X})^\text{T} = 2{\boldsymbol X}^\text{T}({\boldsymbol X}\boldsymbol{w}-\boldsymbol{y})$

$\frac{\partial l}{\partial \boldsymbol{w}}$ 的零点即 $\frac{\partial l}{\partial \boldsymbol{w}}$ 的最小二乘估计 $\boldsymbol{w} = (X^TX)^{-1}X^T\boldsymbol{y}$
