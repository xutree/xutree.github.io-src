...
|  | 标量 | 向量 | 矩阵 |
| :------: | :----: | :----: | :----: |
| 标量 | $\frac{\partial y}{\partial x}$ | $\frac{\partial \mathbf{y}}{\partial x}$ | $\frac{\partial {\bf Y}}{\partial x}$ |
...
在向量微积分中，向量 $\mathbf{y}$ 相对于标量 $x$ 的导数被称为向量 $\mathbf{y}$ 的切向量。
...
\frac{\partial y}{\partial \mathbf{x}^{\text{T}}} =
\left[
...
**例题3**：线性回归问题。$l = \|\mathbf{X}\boldsymbol{w}- \boldsymbol{y}\|^2$。求 $\boldsymbol{w}$ 的最小二乘估计，即求 $\frac{\partial l}{\partial \boldsymbol{w}}$ 的零点。其中 $\boldsymbol{y}$ 是 $m\times 1$ 列向量，$\mathbf{X}$ 是 $m\times n$ 矩阵，$\boldsymbol{w}$ 是 $n\times 1$ 列向量，$l$ 是标量。

**解答**：

改写模平方表达式：$l = (\mathbf{X}\boldsymbol{w}- \boldsymbol{y})^{\text{T}}(\mathbf{X}\boldsymbol{w}- \boldsymbol{y})$

全微分：$dl = (\mathbf{X}d\boldsymbol{w})^\text{T}(\mathbf{X}\boldsymbol{w}-\boldsymbol{y})+(\mathbf{X}\boldsymbol{w}-\boldsymbol{y})^\text{T}(\mathbf{X}d\boldsymbol{w}) = 2(\mathbf{X}\boldsymbol{w}-\boldsymbol{y})^\text{T}\mathbf{X}d\boldsymbol{w}$

对照全微分公式得到：$\frac{\partial l}{\partial \boldsymbol{w}}= (2(\mathbf{X}\boldsymbol{w}-\boldsymbol{y})^\text{T}\mathbf{X})^\text{T} = 2\mathbf{X}^\text{T}(\mathbf{X}\boldsymbol{w}-\boldsymbol{y})$
