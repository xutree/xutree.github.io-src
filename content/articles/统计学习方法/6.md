...
(1) 选定初始点 $w^{(0)}$，取 $\boldsymbol{B_0}$ 为正定对称矩阵，置 $k=0$  
(2) 计算 $g_k=g(w^{(k)})$，若 $\|g_k\|<\epsilon$，则停止计算，得 $w^\star=w^{(k)}$；否则，转到 (3)  
(3) 由 $\boldsymbol{B_k}p_k=-g_k$ 求出 $p_k$  
(4) 一维搜索：求 $\lambda_k$ 使得
$$f(w^{(k)}+\lambda_kp_k)=\min_{\lambda\geq0}f(w^{(k)}+\lambda p_k)$$
(5) 置 $w^{(k+1)}=w^{(k)}+\lambda_k p_k$  
(6) 计算 $g_{k+1}=g(w^{(k+1)})$，若 $\|g_{k+1}\|<\epsilon$，则停止计算，得 $w^\star=w^{(k+1)}$；否则，按照下式求出 $\boldsymbol{B_{k+1}}$
$$\boldsymbol{B_{k+1}}=\boldsymbol{B_k}+\frac{y_ky_k^\text{T}}{y_k^\text{T}\delta_k}-\frac{\boldsymbol{B_k}\delta_k\delta_k^\text{T}\boldsymbol{B_k}}{\delta_k^\text{T}\boldsymbol{B_k}\delta_k}$$
其中，
