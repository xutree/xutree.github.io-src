...
在最优解 $\alpha^\star=\left(\alpha_1^\star,\alpha_2^\star,\cdots,\alpha_N^\star\right)^\text{T}$ 中，$\alpha_i^\star>0$ 的样本点才是支持向量，而对于支持向量 $(x_j,y_j)$ 来说，其一定位于方程 $y_j(w^\ast\cdot x_j + b)=1$ 上，又因为 $|y_j|=1$，故 $y_j=w^\ast\cdot x_j + b$，得出
